...
main:
  ...
  loop:
    ...
    inner_loop:
	  ...
	  beq t1, zero, inner_loop
	  
	  call spend_time
	  stw zero, BUTTON+4(zero)

	  ; t1 is the approximate time of button press execution
	  addi t1, zero, 0b1011001
	  slli t1, t1, 15
	  addi t1, t1, 0b100110011010010

	  ;s0 += t1 % t0 pure
	  while:
		cmpge t2, t1, s2
	  	beq t2, zero, skip_while
		sub t1, t1, s2
		addi s0, s0, 1
	    br while
	  ...
	  ;if t1 > s1 then s1 -= t1
	  cmpge t2, s1, t1
	  beq t2, zero, skip_sub
	  sub s1, s1, t1
	  skip_sub:

	  ; t0 impure -= t1 / t0 pure
